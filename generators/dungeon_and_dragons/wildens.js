...
module.exports = generatorOf(function generator$dungeon_and_dragons$wildens(type) {
	...
	i = Math.floor(this.random() * 10); {
		if (type === 1) {
			...
			if (i < 5) {
				rnd = Math.floor(this.random() * namesMaleFf.length);
				rnd2 = Math.floor(this.random() * namesMaleFl.length);
				names = namesMaleFf[rnd] + namesMaleFl[rnd2];
			} else {
				rnd = Math.floor(this.random() * namesMaleSf.length);
				rnd2 = Math.floor(this.random() * namesMaleSl.length);
