...
module.exports = generatorOf(function generator$places$temples() {
	...
	i = Math.floor(this.random() * 10); {
		if (i < 3) {
			rnd = Math.floor(this.random() * names5.length);
			rnd2 = Math.floor(this.random() * names6.length);
			names = names5[rnd] + " of " + names6[rnd2];
		} else if (i < 5) {
			rnd = Math.floor(this.random() * names5.length);
			rnd2 = Math.floor(this.random() * names7.length);
			rnd3 = Math.floor(this.random() * names8.length);
			rnd4 = Math.floor(this.random() * names9.length);
			rnd5 = Math.floor(this.random() * names10.length);
			names = names5[rnd] + " of " + names7[rnd2] + names8[rnd3] + names9[rnd4] + names10[rnd5];
		} else if (i < 8) {
			rnd = Math.floor(this.random() * names11.length);
			rnd2 = Math.floor(this.random() * names12.length);
			names = "The " + names11[rnd] + " " + names12[rnd2];
		} else {
			rnd = Math.floor(this.random() * names1.length);
			rnd2 = Math.floor(this.random() * names2.length);
			rnd3 = Math.floor(this.random() * names3.length);
			if (rnd < 20) {
				while (rnd3 > 14) {
					rnd3 = Math.floor(this.random() * names3.length);
				}
			}
			rnd4 = Math.floor(this.random() * names2.length);
			if (rnd2 > 4) {
				while (rnd4 > 4) {
					rnd4 = Math.floor(this.random() * names2.length);
				}
			}
			rnd5 = Math.floor(this.random() * names4.length);
