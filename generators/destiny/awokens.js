...
module.exports = generatorOf(function generator$destiny$awokens(type) {
	...
	i = Math.floor(this.random() * 10); {
		rndA = Math.floor(this.random() * nm9.length);
		rndB = Math.floor(this.random() * nm10.length);
		rndE = Math.floor(this.random() * nm12.length);
		if (i % 2 === 0) {
			nmLast = nm9[rndA] + nm10[rndB] + nm12[rndE]
		} else {
			rndC = Math.floor(this.random() * nm11.length);
			rndD = Math.floor(this.random() * nm10.length);
